...
foreign key (JS_ID) references job_seeker(JS_ID) on delete cascade,
foreign key (Com_ID) references company(Com_ID) on delete cascade,
foreign key (js_id, com_id, we_role) references work_experience(js_id, com_id, we_role) on delete cascade
);
...
foreign key (JS_ID) references Job_Seeker(JS_ID) on delete cascade,
foreign key (js_id, Year) references Qualification(js_id, Year) on delete cascade
);

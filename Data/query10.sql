...
WHERE NOT EXISTS (
    ...
    AND (proficiency, domain) NOT IN (select domain, proficiency from skills_used where js_id = 75130 union select proficiency,domain from skills_learnt where js_id = 75130)
);
...
WHERE NOT EXISTS (
    ...
    FROM skills_required
    where skills_required.job_id = job.job_id
    AND (proficiency, domain) NOT IN (select domain, proficiency from skills_used where js_id = 60670 union select proficiency,domain from skills_learnt where js_id = 60670)
)
and job.jstatus != "job_status.filled"
...
select proficiency, domain from skills_required where skills_required.Job_ID = 39824;




-- 5
-- old 5 based on total work experience
SELECT *
FROM job_seeker
left JOIN (
...
WHERE NOT EXISTS (
    SELECT proficiency, domain
    FROM skills_required
    WHERE skills_required.job_id = 34915
...
) AND job_seeker.status != 'employed'
ORDER BY total_exp.total_experience DESC;



-- updated 5 based only on relevent work experience
SELECT job_seeker.*
FROM job_seeker
LEFT JOIN (
    ...
    WHERE sr.job_id = 34915 -- Replace with the actual job ID
    GROUP BY we.js_id
) AS total_exp ON job_seeker.js_id = total_exp.js_id
WHERE NOT EXISTS (
    SELECT proficiency, domain
    FROM skills_required
    WHERE skills_required.job_id = 34915 -- Replace with the actual job ID
    AND (proficiency, domain) NOT IN (
        SELECT domain, proficiency
        FROM skills_used
        WHERE js_id = job_seeker.js_id
...
-- for second query consider Age/Gender/Degree
...
-- Job
 ...
 select * from Job where Job.location = 'Florence, Italy';
 
 select * from job where job.salary >9000000 order by job.salary desc;

-- query 9
WITH CandidateSkills AS (
    SELECT js_id, proficiency, domain
    FROM skills_learnt
    WHERE js_id = 354 -- Replace 'candidate_id' with the actual candidate ID
    UNION
    SELECT js_id, proficiency, domain
    FROM skills_used
    WHERE js_id = 354 -- Replace 'candidate_id' with the actual candidate ID
...
UPDATE company
SET Com_Rating = (0 + ( Com_Rating) * 
                  (SELECT COUNT(*) FROM work_experience WHERE Com_ID = 22617)) /
                 ((SELECT COUNT(*) FROM work_experience WHERE Com_ID = 22617) + 1)
WHERE Com_ID = 22617; -- change @toID

select * from company where com_id=22617;
...
delete from job_seeker where js_id = 29477;
select * from work_experience where js_id = 29477;
select * from qualification where js_id = 29477;
select * from applies where js_id = 55654;
select * from skills_used where js_id = 55654;
